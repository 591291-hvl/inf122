...
numberOfMoons (PlanetarySystem n p) = sum $ map (\(Planet n m) -> toInteger $ length m) p 

atLeastOneMoon :: PlanetarySystem -> [Planet] 
atLeastOneMoon (PlanetarySystem n p) = [ x | x <- p, 0 < (\(Planet n m) -> toInteger $ length m) x]

main = print(atLeastOneMoon solarSystem)
